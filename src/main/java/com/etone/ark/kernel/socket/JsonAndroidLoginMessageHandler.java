 ...
 *
 */
public class JsonAndroidLoginMessageHandler extends AbsSocketMessageHandler<SocketJsonMessage>{
    
    static Logger logger = Logger.getLogger(JsonAndroidLoginMessageHandler.class);
	
	public JsonAndroidLoginMessageHandler(ISocketMessageDecode<SocketJsonMessage> nextDecode) {
        super(nextDecode);
    }
	
    @Override
    public boolean execute(SocketChannel channel, SocketJsonMessage msg) {
        if("androidLogin".equals(msg.getMessageHeader().getCommand())){
            try{
            	...
            	if(info != null){
		    		...
		    		if(error == 4){
		                 ...
		                 event.setMsg("Android设备序列号重复/非法，设备IP："+device.getDeviceIp()+"，设备序列号："+device.getSerialNum());
		                 ClientManager.callbackEvent(event);
		                 CommandServer.responseAndroidLogin(device,null,0);
		                 logger.info("[RESOURCE]["+device.getSerialNum()+"]Android设备序列号重复/非法,IP:"+device.getDeviceIp());
		                 return true;
		    		}else if(error == 3){
		    			 EventExceptionLog event = new EventExceptionLog();
		                 event.setCode(ResultCode.ER_KERNEL_DEVICE_ILLEGAL);
		                 event.setMsg("Android设备参数错误，设备IP："+device.getDeviceIp()+"，设备序列号："+device.getSerialNum());
		                 ClientManager.callbackEvent(event);
		                 CommandServer.responseAndroidLogin(device,null,0);
		                 logger.info("[RESOURCE]["+device.getSerialNum()+"]Android设备参数错误,IP:"+device.getDeviceIp());
		                 return true;
		    		}else if(error == 1 || error == 2){
		                 if(CommandServer.responseAndroidLogin(device,null,1)){
		                	 //获取RTD模块信息
			                 for(RtdInfo.ModuleInfo item:info.getModules()){
			                	 ...
			                	 rtdModule.setBusinessType(Constants.RTD_TYPE_ANDROID); //表示Android设备
			                 ...
			                 logger.info("[RESOURCE]["+device.getSerialNum()+"]Android设备登记,IP："+device.getDeviceIp());
		                     //触发设备变更
                ...
                SocketJsonMessage temp = SocketMessageBuilder.buildJsonMessage(Constants.JSON_PROTOCOL_VERSION,"android", SocketByteMessage.TYPE_RESPONSE, SocketJsonMessage.CONTENT_TYPE_JSON, null);
                if(temp != null){
